...
async function deleteDevice(req, res, next) {
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(deviceId)) return res.status(400).send({ message: 'Invalid place' });
        ...
        return res.status(200).json({ message: 'Devices Deleted Successfully', });
    } catch (error) {
